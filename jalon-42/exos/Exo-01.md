# Exercice 1 : Équations Différentielles ($\bigstar$$\star$$\star$$\star$$\star$)

## Problème

Résoudre rigoureusement l'équation différentielle suivante, définie sur $\mathbb{R}$ :
$$1y''(t) + 3y'(t) + 2y(t) = 1 e^t$$
...
2. **Recherche de la solution particulière :**
   Second membre $d(t) = 1 e^t$. Puisque $1$ n'est pas racine de l'équation caractéristique, on cherche $y_P(t) = A e^t$.
   Dérivées : $y_P'(t) = A e^t$ et $y_P''(t) = A e^t$.
   Injection dans l'équation : $A e^t + 3 A e^t + 2 A e^t = 6 A e^t = 1 e^t$.
   Par identification : $6A = 1 \implies A = \frac{1}{6}$.
   Solution particulière : $y_P(t) = \frac{1}{6} e^t$.

3. **Solution générale complète :**
   $y(t) = \lambda_1 e^{-2t} + \lambda_2 e^{-t} + \frac{1}{6} e^t$.
