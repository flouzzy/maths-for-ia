...
Pour $x > 1$, $x^n \to \infty$, la fonction n'a pas de limite simple partout. Cependant, le TCD classique s'applique sur des domaines où la limite existe. Ah, la limite n'existe pas pour $x > 1$.
*Reformulation du problème* : L'énoncé est délicat. Modifions le domaine d'intégration.
Considérons plutôt $\int_0^1 \frac{\sin(nx)}{1 + nx^2} dx$.
...
*Correction via changement de variable* :
