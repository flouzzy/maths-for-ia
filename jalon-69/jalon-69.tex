...
\title{\Huge JALON 69 : THÉORÈME DE CONVERGENCE DOMINÉE DE LEBESGUE \\ \large Cours magistral, exercices corrigés et travaux pratiques}
...
Imaginez une suite de fonctions $(f_n)$ qui convergent ponctuellement vers une fonction limite $f$. Sous quelles conditions a-t-on l'assurance que l'intégrale des $f_n$ converge vers l'intégrale de $f$ ? La théorie de Riemann exigeait une convergence uniforme, une condition souvent trop rigide et non satisfaite en pratique. Le génie de Lebesgue fut de démontrer que si toute l'agitation des fonctions $f_n$ est contenue en valeur absolue sous une "enveloppe" intégrable $g$, alors l'interversion est toujours valide. La masse de l'intégrale ne peut ni s'échapper vers l'infini géométrique, ni produire de singularités infinies, car la domination $g$ constitue un plafond infranchissable.
...
\begin{tikzpicture}
    ...
    \node[green!50!black] at (3, 3.1) {$g(x)$ (enveloppe intégrable)};
...
\subsection{Énoncé du Théorème de Lebesgue}

\textbf{Théorème de Convergence Dominée (TCD) :}
Soit $(f_n)_{n \in \mathbb{N}}$ une suite de fonctions mesurables (à valeurs réelles ou complexes).
Supposons que :
\begin{enumerate}
    \item \textbf{Convergence simple :} La suite $(f_n)$ converge presque partout vers une fonction mesurable $f$.
    \item \textbf{Domination :} Il existe une fonction $g : X \to [0, +\infty]$ \textbf{intégrable} ($\int g d\mu < +\infty$) telle que :
    $$\forall n \in \mathbb{N}, \quad |f_n| \le g \text{ p.p.}$$
\end{enumerate}
Alors :
\begin{itemize}
    \item $f$ est intégrable ($\int |f| d\mu < +\infty$).
    \item La limite des intégrales est l'intégrale de la limite :
    $$\lim_{n \to \infty} \int_X f_n d\mu = \int_X f d\mu.$$
    \item La convergence a lieu dans $L^1$ : $\lim_{n \to \infty} \int_X |f_n - f| d\mu = 0$.
\end{itemize}

\section{Démonstration du TCD (via le Lemme de Fatou)}

1. \textbf{Intégrabilité de f :}
Puisque $|f_n| \le g$ presque partout pour tout $n$, et que $f_n \to f$ presque partout, on a par passage à la limite :
$$|f| \le g \text{ p.p.}$$
La fonction $g$ étant intégrable, l'intégrale de $g$ est finie. Par croissance de l'intégrale, on obtient $\int |f| d\mu \le \int g d\mu < \infty$, ce qui prouve que $f$ est intégrable.

2. \textbf{Utilisation du Lemme de Fatou :}
La clé est de construire des suites de fonctions positives pour pouvoir appliquer le Lemme de Fatou.
Considérons la suite $h_n = g + f_n$. Puisque $|f_n| \le g$, on a $f_n \ge -g$, et donc $h_n \ge 0$ pour tout $n$.
Les $h_n$ sont mesurables et positives. Appliquons Fatou :
$$\int \liminf_{n \to \infty} h_n d\mu \le \liminf_{n \to \infty} \int h_n d\mu$$
Comme $f_n \to f$ p.p., $\liminf h_n = g + f$ p.p. Ainsi :
$$\int (g + f) d\mu \le \liminf_{n \to \infty} \int (g + f_n) d\mu$$
Par linéarité (car $g$ et $f$ are intégrables) :
$$\int g d\mu + \int f d\mu \le \int g d\mu + \liminf_{n \to \infty} \int f_n d\mu$$
On soustrait $\int g d\mu$ (qui est un nombre réel fini) des deux côtés :
$$\int f d\mu \le \liminf_{n \to \infty} \int f_n d\mu \quad \text{--- (Équation 1)}$$

3. \textbf{Deuxième application de Fatou :}
Considérons maintenant la suite $k_n = g - f_n$. Puisque $f_n \le g$, on a $k_n \ge 0$ pour tout $n$.
On applique Fatou :
$$\int \liminf_{n \to \infty} k_n d\mu \le \liminf_{n \to \infty} \int k_n d\mu$$
$$\int (g - f) d\mu \le \liminf_{n \to \infty} \left( \int g d\mu - \int f_n d\mu \right)$$
La limite inférieure d'une constante moins une suite est la constante moins la \textit{limite supérieure} de la suite. Ainsi :
$$\int g d\mu - \int f d\mu \le \int g d\mu - \limsup_{n \to \infty} \int f_n d\mu$$
On soustrait $\int g d\mu$ :
$$-\int f d\mu \le - \limsup_{n \to \infty} \int f_n d\mu$$
En multipliant par $-1$ (ce qui inverse l'inégalité) :
$$\limsup_{n \to \infty} \int f_n d\mu \le \int f d\mu \quad \text{--- (Équation 2)}$$

4. \textbf{Conclusion :}
En combinant (Équation 1) et (Équation 2), on obtient l'encadrement :
$$\limsup_{n \to \infty} \int f_n d\mu \le \int f d\mu \le \liminf_{n \to \infty} \int f_n d\mu$$
Or on sait toujours que $\liminf \le \limsup$. La seule possibilité est donc que toutes ces grandeurs soient égales, ce qui prouve que la suite des intégrales converge, et que sa limite est bien $\int f d\mu$.
$\blacksquare$
...
print("\nLa fonction g(x) forme un couvercle absolu sur toutes les courbes f_n.")
\end{lstlisting}
