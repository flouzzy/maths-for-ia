...
foreach ($line in $lines) {
    ...
    if ($line.StartsWith("Année")) {
        $current_year = $line
    } elseif ($line.StartsWith("Trimestre")) {
        $current_trimester = $line
        $trimester_context = ""
    } elseif ($line -match "^(Jalon(?:s)? [\d à]+) : (.+)") {
        $j_id = $matches[1]
        $desc = $matches[2]
        
        $short_title = ($desc -split "[,(:]")[0].Trim()
        if ($short_title -match "Livrable IA") {
            $short_title = "Livrable IA"
        }
        
        $filename = "$j_id ($short_title).md"
        $filename = $filename -replace '[\\/*?:"<>|]', '-' # sanitize filename for windows
...
for ($i = 0; $i -lt $jalons.Count; $i++) {
    $jalon = $jalons[$i]
    $content = "# $($jalon.id)`n`n"
    $content += "**$($jalon.year)** > **$($jalon.trimester)**`n`n"
    if ($jalon.context.Trim()) {
        $content += "> *$($jalon.context.Trim())*`n`n"
    }
    $content += "## Description`n$($jalon.desc)`n`n"
    
    if ($jalon.id -match "108") {
        $content += "**Notions liées** : [[Jalon 76 (Propriétés géométriques de l'espace de Hilbert L^2)]] et [[Jalon 105 (Opérateurs adjoints)]].`n`n"
    }
    
    $links = @()
    if ($i -gt 0) {
        $prev_j = $jalons[$i - 1]
        $prev_name = $prev_j.filename.Replace('.md', '')
        $links += "**Précédent** : [[$prev_name]]"
    }
    if ($i -lt $jalons.Count - 1) {
        $next_j = $jalons[$i + 1]
        $next_name = $next_j.filename.Replace('.md', '')
        $links += "**Suivant** : [[$next_name]]"
    }
    
    $content += "---`n"
    $content += ($links -join " | ") + "`n"
    
    $filepath = Join-Path $PWD $jalon.filename
    [System.IO.File]::WriteAllText($filepath, $content, [System.Text.Encoding]::UTF8)
}
Write-Output "Created $($jalons.Count) notes."
