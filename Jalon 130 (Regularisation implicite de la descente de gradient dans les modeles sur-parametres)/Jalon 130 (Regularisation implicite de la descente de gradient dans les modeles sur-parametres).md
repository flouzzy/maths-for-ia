...
- **Le Pont Théorique :** Le phénomène de double descente (abordé au [[Jalon 144 (Le phénomène de double descente)]]) et la généralisation hors-norme des réseaux de neurones profonds (Deep Learning) défient la théorie classique de l'apprentissage statistique de Vapnik-Chervonenkis (voir [[Jalon 136 (Théorie de Vapnik-Chervonenkis)]]). Dans le régime sur-paramétré des LLMs (Large Language Models) ou des CNNs massifs, les modèles ont la capacité de mémoriser n'importe quel bruit aléatoire. S'ils ne le font pas en pratique, c'est grâce au biais inductif de la descente de gradient stochastique (SGD). La SGD agit comme un rasoir d'Ockham mathématique implicite, cherchant activement la variété de dimension minimale ou la fonction la plus régulière qui interpole les données, évitant ainsi les fluctuations violentes (hautes fréquences) caractéristiques de l'overfitting pur.
