...
$output = "# Tableau de Bord : Mathematiques pour l'IA`n`n"
$output += "Bienvenue dans votre plan de formation global. Cochez les jalons au fur et a mesure de votre progression !`n`n"
...
foreach ($line in $lines) {
    ...
    if ($line.StartsWith("Année")) {
        ...
        $filename = "$j_id ($short_title)"
        $filename = $filename -replace '[\\/*?:"<>|]', '-' # sanitize filename for windows
        
        $output += "- [ ] [[$filename]] : $desc`n"
    }
}

$filepath = Join-Path $PWD "Tableau de bord.md"
[System.IO.File]::WriteAllText($filepath, $output, [System.Text.Encoding]::UTF8)
Write-Output "Tableau de bord.md created."
