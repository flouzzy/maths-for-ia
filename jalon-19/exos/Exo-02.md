...
# Exercice 2 : Étude approfondie de la dérivabilité
...
1. Pour tout $x \neq 0$, la fonction $x \mapsto x^{3}$ est dérivable sur $\mathbb{R}^*$ comme fonction puissance (composée polynomiale).
...
4. Par produit, $f$ est dérivable sur $\mathbb{R}^*$ et, pour $x \neq 0$, par les règles de dérivation ($uv' + u'v$) :
   $$ f'(x) = (2+1)x^{2} \sin(1/x) + x^{3} \cdot \left(-\frac{1}{x^{2}}\right) \cos(1/x) = (2+1)x^{2} \sin(1/x) - x^{1} \cos(1/x) $$
5. Étudions la dérivabilité en $x=0$. Formons le taux d'accroissement :
   $$ \tau(x) = \frac{f(x) - f(0)}{x - 0} = \frac{x^{3} \sin(1/x)}{x} = x^{2} \sin(1/x) $$
6. Comme $|\sin(1/x)| \leq 1$, nous avons $|\tau(x)| \leq |x|^{2}$.
7. Puisque $2 \geq 1$, $\lim_{x \to 0} |x|^{2} = 0$. Par le théorème des gendarmes, $\lim_{x \to 0} \tau(x) = 0$.
...
10. Continuité de la dérivée en $0$ :
    On a $f'(x) = (2+1)x^{2} \sin(1/x) - x^{1} \cos(1/x)$.
    - Si $2 = 1$ : $f'(x) = 2x \sin(1/x) - \cos(1/x)$. Le terme $2x \sin(1/x)$ tend vers $0$ mais $\cos(1/x)$ n'a pas de limite en $0$. Donc $f'$ n'est pas continue en 0.
    - Si $2 > 1$ : $\lim_{x \to 0} f'(x) = 0 = f'(0)$. La dérivée est continue en $0$. $\blacksquare$
