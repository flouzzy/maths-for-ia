...
Riemann a démontré un résultat stupéfiant : si une série converge grâce aux compensations exactes entre ses termes positifs et négatifs (ce qu'on appelle la semi-convergence), on peut, simplement en réordonnant l'ordre de sommation, forcer la série à converger vers *n'importe quel nombre réel*, ou même la faire diverger vers l'infini ! Cela signifie que l'addition infinie n'est plus ni commutative, ni associative.
...
En Intelligence Artificielle et en traitement du signal, cette distinction est vitale. Lorsque l'on construit un filtre infini (IIR) ou que l'on effectue des convolutions discrètes de signaux temporels infinis, la garantie mathématique que ces algorithmes n'exploseront pas et ne dépendront pas de l'ordre d'évaluation du code repose entièrement sur la convergence absolue des séries sous-jacentes.
...
**Démonstration détaillée (Zéro Ellipse) :**
